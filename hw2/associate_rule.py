...
def extract_data():
    ...
    with open("hw2/data/data2.dat", "r") as file:
        for line in file:
...
# find the association rules from frequent itemsets
# the association rules should have confidence above a certain threshold
# e.g: for itemset (A, B, C) with support 1500
#      the rules are:
#      A, B -> C with confidence = support(A, B, C) / support(A, B)
#      A, C -> B with confidence = support(A, B, C) / support(A, C)
#      B, C -> A with confidence = support(A, B, C) / support(B, C)
#      A -> B, C with confidence = support(A, B, C) / support(A)
#      B -> A, C with confidence = support(A, B, C) / support(B)
#      C -> A, B with confidence = support(A, B, C) / support(C)
    ...
    for itemset, support in frequent_itemsets.items():
        ...
        for i in range(1, n):
            for antecedent in combinations(items, i):
                ...
                if len(antecedent) == 1:
                    antecedent_support = frequent_single_items.get(antecedent[0], 0)
                elif len(antecedent) == 2:
                    antecedent_support = frequent_double_items.get(antecedent, 0)
                else:
                    antecedent_support = frequent_triple_items.get(antecedent, 0)
                ...
                if confidence >= min_confidence:
                    rule.append((antecedent, consequent, confidence))
    return rule


association_rules = generate_association_rules(
    {**frequent_double_items, **frequent_triple_items}, min_confidence_threshold
)
